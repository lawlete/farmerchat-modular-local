...
export const ChatPanel = forwardRef<ChatPanelHandles, ChatPanelProps>((
  ...
  ref: ForwardedRef<ChatPanelHandles>
) => {
  ...
  const stopRecording = () => {
    ...
    } else if (mediaRecorderRef.current && mediaRecorderRef.current.state === "inactive" && audioChunksRef.current.length > 0) {
      // If already stopped but chunks exist (e.g. programmatic stop before data fully processed by onstop)
      // trigger the onstop logic manually if necessary, or just ensure recording state is false.
      // This case might be redundant if onstop always fires correctly.
       setIsRecording(false);
    } else {
