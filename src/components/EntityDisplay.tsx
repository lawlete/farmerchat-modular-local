...
export const EntityDisplay: React.FC<EntityDisplayProps> = ({ items, defaultExpanded = false }) => {
  ...
  const toggleItemExpansion = (idx: number) => { // Changed parameter name for clarity
  ...
  return (
    <div className="space-y-2 text-xs">
      {items.map((item, index) => ( // 'index' is correctly defined here
        <div key={item.id || index} className="bg-gray-200 dark:bg-gray-600 p-2 rounded shadow">
          <button
            onClick={() => toggleItemExpansion(index)} // Using 'index' from map
