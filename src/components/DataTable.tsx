...
export const DataTable: React.FC<DataTableProps> = ({ items }) => {
  ...
  const allKeys = items.reduce<string[]>((acc, item) => {
    Object.keys(item).forEach(keyVal => { // Changed 'key' to 'keyVal' to avoid conflict if 'key' is a prop
      if (!acc.includes(keyVal)) {
        acc.push(keyVal);
      }
    });
    return acc;
  ...
  const formatHeader = (headerKey: string): string => { // Changed 'key' to 'headerKey'
    let result = headerKey.replace(/([A-Z])/g, ' $1');
    ...
    <div className="overflow-x-auto bg-gray-200 dark:bg-gray-650 rounded-md shadow">
      <table className="min-w-full text-xs text-left text-gray-700 dark:text-gray-300">
        <thead className="bg-gray-300 dark:bg-gray-700 sticky top-0">
          <tr>
            {sortedKeys.map(headerKey => ( // Changed 'key' to 'headerKey'
              <th key={headerKey} scope="col" className="px-3 py-2 font-medium tracking-wider capitalize">
        ...
        <tbody className="bg-gray-100 dark:bg-gray-600 divide-y divide-gray-300 dark:divide-gray-500">
          {items.map((item, index) => (
            <tr key={item.id || index} className="hover:bg-gray-200 dark:hover:bg-gray-550 transition-colors">
              {sortedKeys.map(cellKey => ( // Changed 'key' to 'cellKey'
                <td key={`${item.id || index}-${cellKey}`} className="px-3 py-2 whitespace-nowrap">
                  {formatValue(item[cellKey])}
                </td>
              ))}
            </tr>
          ))}
