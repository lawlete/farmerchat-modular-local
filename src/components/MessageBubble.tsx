...
const renderItemDetails = (item: Record<string, any>): string => {
  ...
  if (item.id && details.length === 0 && !item.name && !item.taskName && !item.description) details.push(\`ID: \${item.id}\`);
  ...
  if (details.length === 0) {
    const fallbackDetails = Object.entries(item)
      .filter(([key]) => key !== 'id' && typeof item[key] !== 'object' && item[key] !== null && item[key] !== undefined && String(item[key]).trim() !== '')
      .slice(0, 2) 
      .map(([key, value]) => \`\${key.charAt(0).toUpperCase() + key.slice(1)}: \${String(value)}\`)
      .join(' | ');
    return fallbackDetails || \`ID: \${item.id || '(sin ID)'}\`;
  }
  return details.join(' | '); 
};
