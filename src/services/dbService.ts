...
export const processCsvData = (csvText: string, entityType: EntityType): any[] => {
  ...
  const results = parsedData.map(row => {
    ...
    // Check if any meaningful data was actually parsed for the row beyond a generated ID
    const meaningfulKeys = Object.keys(entity).filter(k => {
        // If ID was from CSV, it's meaningful. If generated, only count if other data exists.
        if (k === 'id' && !idAssignedFromCsv && Object.keys(entity).length === 1) return false;
